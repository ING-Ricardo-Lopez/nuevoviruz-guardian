...
Describe 'cache.sh'
  ...
  Describe 'get_metadata_hash()'
    setup() {
      ...
      echo "config content" > "$TEMP_DIR/.nvg"
    }
    ...
    It 'returns a hash combining rules and config'
      When call get_metadata_hash "$TEMP_DIR/AGENTS.md" "$TEMP_DIR/.nvg"
      The status should be success
      The length of output should eq 64
    End

    It 'changes when rules file changes'
      hash1=$(get_metadata_hash "$TEMP_DIR/AGENTS.md" "$TEMP_DIR/.nvg")
      echo "new rules" > "$TEMP_DIR/AGENTS.md"
      hash2=$(get_metadata_hash "$TEMP_DIR/AGENTS.md" "$TEMP_DIR/.nvg")
      The value "$hash1" should not eq "$hash2"
    End

    It 'changes when config file changes'
      hash1=$(get_metadata_hash "$TEMP_DIR/AGENTS.md" "$TEMP_DIR/.nvg")
      echo "new config" > "$TEMP_DIR/.nvg"
      hash2=$(get_metadata_hash "$TEMP_DIR/AGENTS.md" "$TEMP_DIR/.nvg")
      The value "$hash1" should not eq "$hash2"
    End
  End
  ...
  Describe 'init_cache()'
    setup() {
      ...
      export CACHE_DIR="$TEMP_DIR/.cache/nvg"
    }
    ...
    It 'creates cache directory structure'
      When call init_cache "AGENTS.md" ".nvg"
      The status should be success
    ...
    It 'creates metadata file'
      init_cache "AGENTS.md" ".nvg" > /dev/null
  ...
  Describe 'is_cache_valid()'
    ...
    It 'returns success when cache is valid'
      When call is_cache_valid "AGENTS.md" ".nvg"
      The status should be success
    End
